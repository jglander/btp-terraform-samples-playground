...
locals {
  integrationsuite_subscription_app_name = [
...
resource "local_file" "output_vars_step1" {
  ...
  content  = <<-EOT
      ...
      integrationsuite_subscription_app_name = "${local.integrationsuite_subscription_app_name}"
      integration_provisioners = ${jsonencode(var.integration_provisioners)}
