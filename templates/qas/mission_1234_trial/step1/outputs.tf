...
output "cf_api_url" {
  value       = lookup(jsondecode(local.cf_env_instance.labels), "API Endpoint", "not found")
  description = "API endpoint of the Cloud Foundry environment."
}

output "cf_landscape_label" {
  value       = local.cf_env_instance.landscape_label
  description = "Landscape label of the Cloud Foundry environment."
}

output "cf_org_id" {
  value       = local.cf_env_instance.platform_id
